                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('workorder.update', $workorder->id_workorder) }}">
                        ...
                        <div class="form-group row mb-3">
                            ...
                            <div class="col-md-6">
                                <input id="id_workorder" type="text"
                                    class="form-control @error('id_workorder') is-invalid @enderror" name="id_workorder"
                                    value="{{ $workorder->id_workorder }}" required readonly>
                                @error('id_workorder')
                        ...
                        <div class="form-group row mb-3">
                            ...
                            <div class="col-md-6">
                                <select id="status" class="form-control @error('status') is-invalid @enderror"
                                    ...
                                    <option value="open" {{ $workorder->status == 'open' ? 'selected' : '' }}>Open
                                    </option>
                                    <option value="in_progress" {{ $workorder->status == 'in_progress' ? 'selected' : ''
                                        }}>In Progress</option>
                                    <option value="closed" {{ $workorder->status == 'closed' ? 'selected' : '' }}>Closed
                                    </option>
                                </select>
                                @error('status')
                                <span class="invalid-feedback" role="alert">
                        ...
                        <div class="form-group row mb-3">
                            ...
                            <div class="col-md-6">
                                <select id="segmen" class="form-control @error('segmen') is-invalid @enderror"
                                    ...
                                    <option value="feeder" {{ $workorder->segmen == 'feeder' ? 'selected' : '' }}>Feeder
                                    </option>
                                    <option value="distribusi" {{ $workorder->segmen == 'distribusi' ? 'selected' : ''
                                        }}>Distribusi</option>
                                </select>
                                @error('segmen')
                                <span class="invalid-feedback" role="alert">
                        ...
                        <div class="form-group row mb-3">
                            ...
                            <div class="col-md-6">
                                <select id="assigned_to" class="form-control @error('assigned_to') is-invalid @enderror"
                                    ...
                                    <option value="{{ $teknisi->id }}" {{ $workorder->assigned_to == $teknisi->id ?
                                        'selected' : '' }}>
                                        {{ $teknisi->name }}
                                    </option>
                                    @endforeach
                        ...
                        <div class="form-group row mb-3">
                            ...
                            <div class="col-md-6">
                                <select id="id_tiket" class="form-control @error('id_tiket') is-invalid @enderror"
                                    ...
                                    <option value="{{ $ticket->id_tiket }}" {{ $workorder->id_tiket == $ticket->id_tiket
                                        ? 'selected' : '' }}>
                                        {{ $ticket->id_tiket }}
                                    </option>
                                    @endforeach
                        ...
                        <div class="form-group row mb-3">
                            ...
                            <div class="col-md-6">
                                <div id="material-usage-container">
                                    @if($workorder->materials && count($workorder->materials) > 0)
                                    @foreach($workorder->materials as $usage)
                                    <div class="material-entry mb-2 d-flex">
                                        <select name="material_id[]" class="form-control mr-2" style="width: 60%">
                                            ...
                                            <div>
                                                {{ $material->id_material }} - Quantity: {{ $material->pivot->qty_used }}
                                                - Total: ${{ $material->pivot->total_price }}
                                            </div>
                                            @endforeach
                                        </select>
                                        <input type="number" name="material_quantity[]" class="form-control mr-2"
                                            style="width: 30%" placeholder="Qty" value="{{ $usage->pivot->qty_used }}">
